...
import os
from webui_pages.xunfei_tts import *
from webui_pages.utils import *
from webui_pages.whisper_stt import *
from webui_pages.openai_tts import *
...
my_modal = Modal(title=" ", key="modal_key",max_width=900)
...
def dialogue_page(api: ApiRequest, is_lite: bool = False):
    ...
    default_model = api.get_default_llm_model()[1]

    if not chat_box.chat_inited:
        st.toast(
            f"欢迎使用 [`青岛科技大学智能问答系统`] ! \n\n"
            f"当前运行的模型`qwen-api`, 您可以开始提问了."
        )
        ...
        video_path = os.path.join(directory,"hello_video.mp4")
        # with my_modal.container():
        #     # 使用 st.video 播放 MP4 视频
        #     st.video(video_path, autoplay=True)

    # with my_modal.container():
    #     # 使用 st.video 播放 MP4 视频
    #     st.video('D:\whj\\v1\hello_video.mp4', autoplay=True)
    ...
    if prompt:= prompt or prompt_button:
        if parse_command(text=prompt, modal=modal):  # 用户输入自定义命令
            ...
            if dialogue_mode == "LLM 对话":
                ...
                # path = openai_tts(text, "../result.wav")
                dashscope_tts(text)
                # st.audio("result.wav", format="audio/wav",autoplay=True)
                # 获取当前文件的绝对路径
                file_path = os.path.abspath(__file__)
                # 获取当前文件所在的目录
                directory = os.path.dirname(file_path)

                gif_path = os.path.join(directory, 'Haiyan.gif')
                out_video_path = os.path.join(directory, 'output_video.mp4')
                result_path = os.path.join(directory,"result.wav")

                audio_clip = AudioFileClip(result_path)

                # 加载 GIF 文件作为视频剪辑，并设置循环播放时长
                video_clip = VideoFileClip(gif_path).loop(duration=audio_clip.duration)

                # 将音频剪辑设置到视频剪辑上
                final_clip = video_clip.set_audio(audio_clip)

                # 导出为 MP4 文件
                final_clip.write_videofile(out_video_path, codec='libx264', audio_codec='aac')

                st.video(out_video_path, autoplay=True)
                ...
                text = ""
                ans = ""
                for d in api.agent_chat(prompt + "，简要说明，100字以内",
                                        history=history,
                                        model=llm_model,
                                        prompt_name=prompt_template_name,
                                        temperature=temperature,
                ...
                # path = openai_tts(ans, "../result.wav")
                # st.audio("result.wav", format="audio/wav",autoplay=True)
                # 获取当前文件的绝对路径
                file_path = os.path.abspath(__file__)
                # 获取当前文件所在的目录
    ...
    with st.sidebar:
        ...
        st.caption(
            f"""
            <p align="right" style="color: blue;">© 2024 青岛科技大学数据科学学院</span><br>
            <span style="color: blue;">版权所有</span></p>
            """,
